...
import { Trash2, Plus, FileText, HelpCircle, Award, Save, PenSquare, CheckCircle2 } from 'lucide-react';
...
export default function PollForm({ onSubmitAction, initialData, isLoading = false }: PollFormProps) {
  const defaultValues: Omit<Poll, 'id'> = initialData || {
    title: '',
    description: '',
    questions: [
      {
  ...
  // Create a field array for each question's answers
  // This is done at the top level, not inside the map function
  const answersFieldArrays = questionFields.map((field, index) => {
    return {
  ...
  const onFormSubmit: SubmitHandler<Omit<Poll, 'id'>> = (data) => {
    onSubmitAction(data);
  };
            ...
            <div className="space-y-6 mt-8">
              <div className="flex justify-between items-center bg-gradient-to-r from-gray-50 to-gray-100 p-4 rounded-xl shadow-sm">
                ...
                <Button
                  ...
                  className="bg-[#009688] hover:bg-[#00796b] text-white border-none flex items-center gap-2 transition-all px-4 py-2 rounded-lg shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                >
                  <Plus className="h-5 w-5" /> إضافة سؤال جديد
                </Button>
              </div>

              <div className="space-y-6">
                {questionFields.map((field, questionIndex) => {
                  // Find the corresponding answers field array
                  const answersFieldArray = answersFieldArrays.find(a => a.index === questionIndex)?.fields;
                  
                  if (!answersFieldArray) return null;
                    ...
                    >
                      <div className="bg-gradient-to-r from-gray-50 to-gray-100 p-4 flex justify-between items-center border-b border-gray-200">
                        ...
                        <Button
                          ...
                          className="text-red-500 hover:bg-red-50 h-9 w-9 p-0 rounded-full transition-all"
                        ...
                        <div className="space-y-4 mt-6">
                          <div className="flex justify-between items-center bg-gradient-to-r from-gray-50 to-gray-100 p-3 rounded-lg">
                            <Label className="font-medium flex items-center gap-2 text-[#1e1e2d]">
                              ...
                              الإجابات ({answersFieldArray.fields.length})
                            </Label>
                            <Button
                              type="button"
                              variant="outline"
                              ...
                              onClick={() =>
                                answersFieldArray.append({
                                  id: answersFieldArray.fields.length,
                                  text: '',
                                  points: 0,
                                })
                              }
                              className="bg-[#1e1e2d] hover:bg-[#2d2d3d] text-white border-none flex items-center gap-1 text-xs py-1 px-3 rounded-lg transition-all shadow-sm hover:shadow-md"
                            >
                              <Plus className="h-3 w-3" /> إضافة إجابة
                            </Button>
                          </div>

                          <div className="space-y-3 mt-3 bg-gray-50 p-4 rounded-lg">
                            {answersFieldArray.fields.map((answerField, answerIndex) => (
                              <div 
                                key={answerField.id} 
                              ...
                              >
                                ...
                                <Button
                                  ...
                                  className="text-red-500 hover:bg-red-50 h-8 w-8 p-0 rounded-full transition-all flex-shrink-0"
                                  disabled={answersFieldArray.fields.length <= 2}
                                >
