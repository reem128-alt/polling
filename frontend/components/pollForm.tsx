...
import { useForm, useFieldArray, SubmitHandler, UseFormRegister, FieldErrors } from 'react-hook-form';
import { Poll } from '@/lib/services/poll';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { Card, CardContent, CardFooter, CardHeader, CardTitle } from '@/components/ui/card';
import { FileText, HelpCircle, Award, Save, PenSquare, CheckCircle2, Trash2, PlusCircle } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 p-4 flex justify-between items-center border-b border-gray-200">
        <Label htmlFor={`questions.${index}.text`} className="font-bold text-lg flex items-center gap-3">
          <span className="bg-[#009688] text-white w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shadow-md">
            {index + 1}
          </span>
          <span className="text-[#1e1e2d]">السؤال {index + 1}</span>
        </Label>
        {canRemoveQuestion && (
          <Button
            type="button"
            onClick={() => onRemoveQuestion(index)}
            className="bg-[#1e1e2d] hover:bg-[#141422] text-white text-sm py-2 px-4 rounded-lg transition-all shadow-sm hover:shadow-md flex items-center gap-1"
          >
      ...
      <div className="p-5 bg-white">
        ...
        <div className="space-y-4 mt-6">
          ...
          <div className="space-y-3 mt-3 bg-gray-50 p-4 rounded-lg">
            {Array.from({ length: answersCount }).map((_, answerIndex) => (
              <div
              ...
              >
                ...
                <div className="w-full sm:flex-1 relative">
                  <Input
                    {...register(`questions.${index}.answers.${answerIndex}.text` as const, {
                ...
                <div className="relative w-full sm:w-24 mt-2 sm:mt-0">
                  <Input
                    type="number"
                    {...register(`questions.${index}.answers.${answerIndex}.points` as const, {
...
export default function PollForm({ onSubmitAction, initialData, isLoading = false, pollId }: PollFormProps) {
  const mapPollToFormData = (pollData: Omit<Poll, 'id'>): PollFormData => ({
    title: pollData.title,
    description: pollData.description,
    questions: pollData.questions.map((question, qIndex) => ({
      id: qIndex,
      text: question.text,
  ...
  const defaultBlankForm: PollFormData = {
    title: '',
    description: '',
    questions: [
      {
  ...
  const defaultValues: PollFormData = initialData ? mapPollToFormData(initialData) : defaultBlankForm;
              ...
              <div className="space-y-6">
                {questionFields.map((field, questionIndex) => (
                  <QuestionBlock
                    ...
                    register={register}
                    errors={errors}
                    canRemoveQuestion={questionFields.length > 1}
                    onRemoveQuestion={removeQuestion}
                    initialAnswersCount={
                      Array.isArray((questionFields[questionIndex] as any)?.answers)
                        ? (questionFields[questionIndex] as any).answers.length
                        : 2
                    }
                  />
                ))}
